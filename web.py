...
def read_ID():
    try:
        with open(ID_FILE_PATH, 'r', encoding='utf-8') as ID_file:
            ...
            secret_id = tencent_api_config.get("secret_id", "")[:4] + '*' * (
                        len(tencent_api_config.get("secret_id", "")) - 8) + tencent_api_config.get("secret_id", "")[-4:]
            secret_key = tencent_api_config.get("secret_key", "")[:4] + '*' * (
                        len(tencent_api_config.get("secret_key", "")) - 8) + tencent_api_config.get("secret_key", "")[
                                                                             -4:]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        print("可以在启动的时候输入一个数字作为web界面的端口号，否则默认为9876")
